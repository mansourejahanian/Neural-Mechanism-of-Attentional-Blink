function dist = fl_distanceeucl(data,condid,varargin)
...
%
% example: 
%  d = fl_distanceeuclcv(data,condid,'numpermutation',10,'verbose',2); 
%
...
%             end
    ...
    d = d + distanceeucl(traindata,method);
%     if conditiongen
...
function d = distanceeucl(traindata,method)  
...
switch method
    ...
    case 'pairwise'
        ...
        for i = 1:length(x)
            d(:,i) = sum( (traindata{x(i)}-traindata{y(i)}).*(traindata{x(i)}-traindata{y(i)}))'; %cross-validated Euclidean distance
        end
