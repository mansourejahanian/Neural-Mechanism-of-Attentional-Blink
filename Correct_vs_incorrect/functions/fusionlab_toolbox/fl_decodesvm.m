function dec = fl_decodesvm(data,condid,varargin)
...
%
% example: 
%  d = fl_decodesvm(data,condid,'numpermutation',10,'verbose',2); 
%
...
[~]             = fl_inputparser(varargin,[],0, {'numpermutation' 'kfold' 'method' 'whiten' 'dvweighted' 'verbose'} ); %check is all parameters valid
...
method          = fl_inputparser(varargin,'method','pairwise',{'pairwise','temporalgen','multiclass'}); 
whiten          = fl_inputparser(varargin,'whiten',true);
dvweighted      = fl_inputparser(varargin,'dvweighted',false);
...
switch method 
    ...
    case 'temporalgen'
        ...
        [tndx,ssize,nslices] = fl_sliceblocks(ntimes,slicesize);
